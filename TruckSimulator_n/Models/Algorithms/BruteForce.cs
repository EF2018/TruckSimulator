...
namespace TruckSimulator_n
{
    public class BruteForce: Builder, IRouteSearcher
    {
        ...
        public BruteForce(List<Cargo> arrayPoints, Vehicle truck):base(arrayPoints, truck)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<IPoint> BuildBestRoute()
        {
            _log.WriteDataToLog(DateTime.Now.ToString() + " "+Truck.Name+": start find best route");
            this.GetBestRoute();
            _log.WriteDataToLog(DateTime.Now.ToString() + " "+Truck.Name + ": finding finished. Best route is "+BuildRouteByPoints(BestCombinationArray[0]).Count);
            _log.Close();
        ...
        private void CompareWithBestRoutes()
        {
            ...
            _log.WriteDataToLog(Truck.Name+ BuildArrayPointInString()+"="+newroute.ToString());
        ...
        private string BuildArrayPointInString()
        {
            string str="";
            for (int i = 0; i < ArrayPointsTemp.Count; i++)
            {
                str = str+"->"+ArrayPointsTemp[i].Name;
            }
            return str;
        }
